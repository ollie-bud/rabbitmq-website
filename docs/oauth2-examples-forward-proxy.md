...
To start the forward proxy, run the following command:

```
make start-forward-proxy
```
...
Take into account, that the management UI, via the browser, goes straight to keycloak. 
In other words, it does not go via the forward-proxy. If you want the management UI to
go via the forward-proxy you have to configure the browser.

However, upon a successful authentication, RabbitMQ receives a token from Keycloak. 
To validate this token, RabbitMQ connects to keycloak via the forward-proxy, to 
download the signing keys used to validate the token. 
